...
static int configure_serial_interface(struct inv_imu_device *s)
{
    ...
    case UI_I2C:
        ...
        status = inv_imu_read_reg(s, INTF_CONFIG1, 1, &value);  // Reset status here
        if (status != 0) {
        ...
        status = inv_imu_write_reg(s, INTF_CONFIG1, 1, &value);  // Reset status before writing
        if (status != 0) {
            return status;  // Early return on error
        }
        break;
    ...
    default:
        return INV_ERROR_BAD_ARG;  // Directly return the error
    }
